...
class BaseSource(ABC):
    ...
    def fetch_text(self, url: str, *, params: dict | None = None, respect_robots: bool = True,
                    method: str = "GET", json_data: dict | list | None = None,
        ...
        encoded_params = urlencode(sorted((params or {}).items()), doseq=True)
        encoded_body = json.dumps(json_data, sort_keys=True, separators=(",", ":")) if json_data is not None else ""
        cache_key = f"{method} {url}?{encoded_params}#{encoded_body}"
        if self.use_cache:
    ...
    def fetch_json(self, url: str, *, params: dict | None = None, respect_robots: bool = True,
        ...
        return json.loads(self.fetch_text(
            url, params=params, respect_robots=respect_robots, method=method,
            json_data=json_data, ttl_seconds=ttl_seconds,
        ))
