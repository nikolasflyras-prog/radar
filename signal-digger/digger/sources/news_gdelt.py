...
class NewsGdeltSource(BaseSource):
    ...
    def _search(self, query: str, timespan_days: int, max_records: int) -> list[dict]:
        ...
        search_config = self.config.get("news_search", {})
        attempts = max(0, int(search_config.get("gdelt_retry_attempts", 2)))
        base_backoff = max(0.0, float(search_config.get("gdelt_retry_backoff_seconds", 5)))
        text = ""
