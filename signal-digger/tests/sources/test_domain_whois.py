...
def test_source_only_applies_to_company_targets():
    source = DomainWhoisSource(_config(), cache=None, use_cache=False)

    assert source.applies_to(Target(mode="company", query="Acme"))
    assert not source.applies_to(Target(mode="sector", query="semiconductor packaging"))
    assert not source.applies_to(Target(mode="person", query="Jane Chen"))
...
@responses.activate
def test_collect_excludes_a_305_day_old_change_with_default_90_day_window(tmp_path):
    now = datetime.now(timezone.utc)
    responses.add(
        responses.GET, "https://rdap.org/domain/acmeinc.com",
        json={"events": [
            {"eventAction": "registration", "eventDate": (now - timedelta(days=4000)).isoformat()},
            {"eventAction": "last changed", "eventDate": (now - timedelta(days=305)).isoformat()},
        ]}, status=200,
    )
    config = _config()
    config["domain_whois"] = {"recent_registration_days": 90, "recent_change_days": 90}
    source = DomainWhoisSource(config, cache=ResponseCache(tmp_path / "cache.db"), use_cache=False)
    with mock.patch("socket.getaddrinfo", side_effect=_fake_getaddrinfo):
        result = source.collect(Target(mode="company", query="Acme Inc"))

    assert not [finding for finding in result.findings if finding.raw["domain"] == "acmeinc.com"]
