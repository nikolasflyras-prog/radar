...
def slugs_for(config: dict, target: Target) -> dict[str, list[str]]:
    ...
    overrides = boards.get("slug_overrides", {})
    match = overrides.get(target.query)
    if match:
        return {"greenhouse": list(match.get("greenhouse", [])), "lever": list(match.get("lever", []))}
    if target.mode == "sector":
        # A sector phrase cannot be reliably transformed into an employer's
        # Greenhouse/Lever slug. Run only explicitly configured boards instead
        # of issuing two guaranteed-noisy guesses.
        return {
            "greenhouse": list(boards.get("greenhouse_slugs", [])),
            "lever": list(boards.get("lever_slugs", [])),
        }
